...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const fields = ['name','code','location_type','parent_id','address','city','county','postal_code','is_active'];
    const sets: string[] = []; const vals: unknown[] = []; let i = 1;
    for (const f of fields) {
      if (body[f] !== undefined) { sets.push(`${f}=$${i++}`); vals.push(body[f]); }
    }
    if (sets.length) { sets.push('updated_at=NOW()'); vals.push(id); await query(`UPDATE locations SET ${sets.join(',')} WHERE id=$${i}`, vals); }
